...
class AdaBelief(nn.OptimizerBase):
    ...
    def get_weights(self):
        return [self.iterations] + list(self.ms_dict.values()) + list(
            self.vs_dict.values())
    ...
    def initialize_variables(self,
        ...
        with tf.variable_scope(self.name):
            ms = {
                v.name:
                tf.get_variable(f'ms_{v.name}'.replace(':', '_'),
                                v.shape,
                                dtype=v.dtype,
            ...
            vs = {
                v.name:
                tf.get_variable(f'vs_{v.name}'.replace(':', '_'),
                                v.shape,
                                dtype=v.dtype,
                                initializer=tf.initializers.constant(0.0),
                                trainable=False)
            ...
            if self.lr_dropout != 1.0:
                ...
                self.lr_rnds_dict.update({
                    v.name: rnd
                    for v, rnd in zip(trainable_weights, lr_rnds)
                })
        if e: e.__exit__(None, None, None)
    ...
    def get_update_op(self, grads_vars):
        ...
        for i, (g, v) in enumerate(grads_vars):
            ...
            v_diff = -lr * m_t / (tf.sqrt(v_t) +
                                  np.finfo(g.dtype.as_numpy_dtype).resolution)
            if self.lr_dropout != 1.0:
