...
class UNetPatchDiscriminator(nn.ModelBase):
    ...
    def on_build(self, patch_size, in_ch, base_ch=16, use_fp16=False):
        ...
        for i, (kernel_size, strides) in enumerate(layers):
            ...
            self.upconvs.insert(
                0,
                nn.Conv2DTranspose(level_chs[i] *
                                   (2 if i != len(layers) - 1 else 1),
                                   level_chs[i - 1],
                                   kernel_size=kernel_size,
                                   strides=strides,
                                   padding='SAME',
        ...
        self.out_conv = nn.Conv2D(level_chs[-1] * 2,
                                  1,
                                  kernel_size=1,
                                  padding='VALID',
                                  dtype=conv_dtype)

        self.center_out = nn.Conv2D(level_chs[len(layers) - 1],
                                    1,
                                    kernel_size=1,
                                    padding='VALID',
                                    dtype=conv_dtype)
        self.center_conv = nn.Conv2D(level_chs[len(layers) - 1],
    ...
    def forward(self, x):
        ...
        center_out, x = self.center_out(x), tf.nn.leaky_relu(
            self.center_conv(x), 0.2)
