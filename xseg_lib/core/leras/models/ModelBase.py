...
class ModelBase(nn.Saveable):
    ...
    def xor_list(self, lst1, lst2):
        return [
            value for value in lst1 + lst2
            if (value not in lst1) or (value not in lst2)
        ]
    ...
    def summary(self):
        ...
        layers_params = [
            int(np.sum(np.prod(w.shape) for w in l.get_weights()))
            for l in layers
        ]
        ...
        for i in range(len(delim_str) - len_model_name_str):
            model_name_str += " " if i != (len(delim_str) -
                                           len_model_name_str - 2) else " |"
        ...
        for i in range(len(delim_str) - len_total_params_str):
            total_params_str += " " if i != (len(delim_str) -
                                             len_total_params_str -
                                             2) else " |"
