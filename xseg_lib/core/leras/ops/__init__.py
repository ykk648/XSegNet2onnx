        ...
        with nn.tf.device('/CPU:0'):
            ...
            for x, value in tuples:
                if isinstance(value, nn.tf.Operation) or \
                    ...
                    assign_placeholder = nn.tf.placeholder(x.dtype.base_dtype,
                                                           shape=[None] *
                                                           value.ndim)
...
def max_pool(x, kernel_size=2, strides=2):
    if nn.data_format == "NHWC":
        return tf.nn.max_pool(x, [1, kernel_size, kernel_size, 1],
                              [1, strides, strides, 1],
                              'SAME',
                              data_format=nn.data_format)
    else:
        return tf.nn.max_pool(x, [1, 1, kernel_size, kernel_size],
                              [1, 1, strides, strides],
                              'SAME',
                              data_format=nn.data_format)
...
def random_binomial(shape, p=0.0, dtype=None, seed=None):
    ...
    return array_ops.where(
        ...
        array_ops.ones(shape, dtype=dtype), array_ops.zeros(shape,
                                                            dtype=dtype))
...
def gaussian_blur(input, radius=2.0):
    ...
    def make_kernel(sigma):
        ...
        kernel_1d = np.array(
            [gaussian(x, mean, sigma) for x in range(kernel_size)])
...
def style_loss(target,
    ...
    def sd(content, style, loss_weight):
        ...
        c_mean, c_var = tf.nn.moments(content,
                                      axes=nn.conv2d_spatial_axes,
                                      keep_dims=True)
        s_mean, s_var = tf.nn.moments(style,
                                      axes=nn.conv2d_spatial_axes,
                                      keep_dims=True)
