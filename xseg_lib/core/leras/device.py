...
class Devices(object):
    ...
    def _get_tf_devices_proc(q: multiprocessing.Queue):
        ...
        if sys.platform[0:3] == 'win':
            compute_cache_path = Path(
                os.environ['APPDATA']) / 'NVIDIA' / ('ComputeCache_ALL')
            ...
            if dev_type in ['GPU', 'DML']:
                ...
                if len(dev_desc) != 0:
                    if dev_desc[0] == '{':
                        ...
                        for param, value in (v.split(':')
                                             for v in dev_desc.split(',')):
                            param = param.strip()
                            value = value.strip()
                            if param == 'name':
                                dev_name = value
                ...
                physical_devices_f[dev_idx] = (dev_type, dev_name,
                                               dev.memory_limit)
    ...
    def initialize_main_env():
        ...
        p = multiprocessing.Process(target=Devices._get_tf_devices_proc,
                                    args=(q, ),
                                    daemon=True)
        p.start()
        p.join()
    ...
    def getDevices():
        if Devices.all_devices is None:
            if int(os.environ.get("NN_DEVICES_INITIALIZED", 0)) != 1:
                raise Exception(
                    "nn devices are not initialized. Run initialize_main_env() in main process."
                )
            devices = []
