...
from xseg_lib.core.leras.device import Devices
...
class nn():
    ...
    def initialize(device_config=None, floatx="float32", data_format="NHWC"):
        ...
        if nn.tf is None:
            ...
            if len(device_config.devices) != 0:
                if sys.platform[0:3] == 'win':
                    ...
                    if all([
                        ...
                        devices_str = "_" + device_config.devices[
                            0].name.replace(' ', '_')
                    else:
                        devices_str = ""
                    ...
                    compute_cache_path = Path(
                        os.environ['APPDATA']) / 'NVIDIA' / ('ComputeCache' +
                                                             devices_str)
            ...
            import xseg_lib.core.leras.ops
            import xseg_lib.core.leras.layers
            import xseg_lib.core.leras.initializers
            import xseg_lib.core.leras.optimizers
            import xseg_lib.core.leras.models
            import xseg_lib.core.leras.archis
    ...
    @staticmethod
    def ask_choose_device_idxs(choose_only_one=False,
                               allow_cpu=True,
        ...
        if suggest_all_gpu:
            ...
            best_device_indexes = [
                device.index for device in devices.get_equal_devices(
                    devices.get_best_device())
            ]
        else:
        ...
        while True:
            try:
                if choose_only_one:
                    choosed_idxs = io.input_str("Which GPU index to choose?",
                                                best_device_indexes)
                else:
                    choosed_idxs = io.input_str("Which GPU indexes to choose?",
                                                best_device_indexes)
                ...
                if choose_only_one:
                    ...
                    if all(
                        [idx in all_devices_indexes for idx in choosed_idxs]):
                        break
            except:
                pass
        io.log_info("")
    ...
    class DeviceConfig():
        ...
        def GPUIndexes(indexes):
            if len(indexes) != 0:
                devices = Devices.getDevices().get_devices_from_index_list(
                    indexes)
            else:
                devices = []
