...
class Conv2DTranspose(nn.LayerBase):
    ...
    def build_weights(self):
        ...
        if self.use_bias:
            ...
            self.bias = tf.get_variable("bias", (self.out_ch, ),
                                        dtype=self.dtype,
    ...
    def forward(self, x):
        ...
        if nn.data_format == "NHWC":
            ...
            output_shape = tf.stack(
                (tf.shape(x)[0],
                 self.deconv_length(w, self.strides, self.kernel_size,
                                    self.padding),
                 self.deconv_length(h, self.strides, self.kernel_size,
                                    self.padding), self.out_ch))
    ...
    def deconv_length(self, dim_size, stride_size, kernel_size, padding):
        ...
        if dim_size is None:
            return None
        if padding == 'VALID':
            dim_size = dim_size * stride_size + max(kernel_size - stride_size,
                                                    0)
        elif padding == 'FULL':
