...
class AdaIN(nn.LayerBase):
    ...
    def build_weights(self):
        ...
        self.weight1 = tf.get_variable("weight1", (self.mlp_ch, self.in_ch),
                                       dtype=self.dtype,
                                       initializer=kernel_initializer)
        self.bias1 = tf.get_variable("bias1", (self.in_ch, ),
                                     dtype=self.dtype,
                                     initializer=tf.initializers.zeros())
        self.weight2 = tf.get_variable("weight2", (self.mlp_ch, self.in_ch),
                                       dtype=self.dtype,
                                       initializer=kernel_initializer)
        self.bias2 = tf.get_variable("bias2", (self.in_ch, ),
                                     dtype=self.dtype,
                                     initializer=tf.initializers.zeros())
    ...
    def forward(self, inputs):
        ...
        x_std = tf.math.reduce_std(
            x, axis=nn.conv2d_spatial_axes, keepdims=True) + 1e-5
