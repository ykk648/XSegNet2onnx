...
class InteractBase(object):
    ...
    def destroy_window(self, wnd_name):
        if wnd_name in self.named_windows:
            ...
            if wnd_name == self.focus_wnd_name:
                self.focus_wnd_name = list(
                    self.named_windows.keys())[-1] if len(
                        self.named_windows) != 0 else None
    ...
    def process_messages(self, sleep_time=0):
        callbacks = self.process_messages_callbacks.get(
    ...
    def add_key_event(self, wnd_name, ord_key, ctrl_pressed, alt_pressed,
                      shift_pressed):
        if wnd_name not in self.key_events:
            self.key_events[wnd_name] = []
        self.key_events[wnd_name] += [
            (ord_key, chr(ord_key) if ord_key <= 255 else chr(0), ctrl_pressed,
    ...
    def input_in_time(self, str, max_time_sec):
        ...
        p = multiprocessing.Process(target=self.input_process,
                                    args=(sys.stdin.fileno(), sq, str))
        p.daemon = True
        p.start()
        t = time.time()
        inp = False
    ...
    def input_skip_pending(self):
        ...
        """
        p = multiprocessing.Process(target=self.input_process_skip_pending,
...
class InteractDesktop(InteractBase):
    ...
    def on_process_messages(self, sleep_time=0):
        ...
        if has_capture_keys and ord_key != -1:
            self.add_key_event(self.focus_wnd_name, ord_key, False, False,
                               shift_pressed)
